  ...
  const reviewRequest = async (status, _id) => {
    try {
      const res = await axios.patch(
        `${API_BASE_URL}/api/users/review/request/${status}/${_id}`,
        {},
  ...
  const fetchRequest = async () => {
    try {
      const res = await axios.get(
        `${API_BASE_URL}/api/users/request/received`,
        { withCredentials: true }
      );
    ...
    <div className="text-center my-10">
      ...
      {requests.map((request) => {
        ...
        return (
          <div
            key={_id}
            className="flex justify-between items-center m-4 p-4 border rounded-lg bg-base-200 w-2/3 mx-auto"
          >
            <div>
              <img
                alt="photo"
